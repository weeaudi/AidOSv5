...
check_file() {
	...
	while IFS= read -r line || [ -n "$line" ]; do
		...
		if [[ "$line" == *$'\t'* ]] && ! [[ "$line" =~ \;\ *STYLE-IGNORE\ TABS ]]; then
			echo "$f:$n: TABS: tab character found"
			status=1
		fi
		# trailing whitespace
		if [[ "$line" =~ [[:space:]]$ ]] && ! [[ "$line" =~ \;\ *STYLE-IGNORE\ TRAILWS ]]; then
			echo "$f:$n: TRAILWS: trailing whitespace"
			status=1
		fi
		# line length
		if ((${#line} > 100)) && ! [[ "$line" =~ \;\ *STYLE-IGNORE\ LONGLINE ]]; then
			echo "$f:$n: LONGLINE: >100 cols"
			status=1
		fi
		# label case
		if [[ "$line" =~ ^[A-Za-z0-9_]+:\ *$ ]]; then
			lbl="${line%:}"
			if [[ "$lbl" != "${lbl,,}" ]] && ! [[ "$line" =~ \;\ *STYLE-IGNORE\ LABELCASE ]]; then
				echo "$f:$n: LABELCASE: labels must be lowercase"
				status=1
			fi
		fi
		...
		if [[ -n "$stripped" && "${stripped:0:1}" != ';' && "${stripped:0:1}" != '.' && ! "$line" =~ ^[a-z0-9_]+:\ *$ ]]; then
			...
			if [[ "$stripped" =~ ^[A-Za-z0-9_]+:[[:space:]]+((([Dd][Bb])|([Dd][Ww])|([Dd][Dd])|([Dd][Qq]))|([Rr][Ee][Ss][BbWwDdQq])|([Tt][Ii][Mm][Ee][Ss]))([[:space:]]|$) ]]; then
				continue
			fi
			...
			if ((indent != 4)) && ! [[ "$line" =~ \;\ *STYLE-IGNORE\ INDENT ]]; then
				echo "$f:$n: INDENT: instructions indented by 4 spaces"
				status=1
			fi
			mnem="${stripped%%[[:space:]]*}"
